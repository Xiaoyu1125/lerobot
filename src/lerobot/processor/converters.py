...
def _extract_complementary_data(batch: dict[str, Any]) -> dict[str, Any]:
    ...
    # Skip task if it's a tensor (e.g., from streaming dataset with accelerate)
    # In this case, task_index is used instead
    task = batch.get("task")
    import torch
    if task is not None and isinstance(task, torch.Tensor):
        task_key = {}
    else:
        task_key = {"task": task} if task is not None else {}

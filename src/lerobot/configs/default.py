...
class DatasetConfig:
    ...
    # Streaming shuffle and storage tuning. ``raw_batch_size`` and
    # ``lance_decode_device`` are only used for local Lance datasets.
    shuffle_buffer_size: int = 1000
    raw_batch_size: int | None = None
    lance_decode_device: str | None = "cpu"
    # None falls back to the training DataLoader worker count.
    max_num_shards: int | None = None
    # Optional independent seed for streaming order; defaults to the training seed.
    seed: int | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.raw_batch_size is not None and self.raw_batch_size < 1:
            raise ValueError("raw_batch_size must be at least 1 when set")
        if self.max_num_shards is not None and self.max_num_shards < 1:
            raise ValueError("max_num_shards must be at least 1 when set")
